...
class LocalBackend(Backend):
    ...
    def release_stalled_tasks(self, topic: int) -> int:
        ...
        for msg in self.messages:
            ...
            if not msg['acquired']: continue
            if msg['visibility_timeout'] + msg['acquire_time'] > now: continue
    ...
    def get(self, topic: int) -> "Message":
        ...
        now = time.time()
        for msg in self.messages:
            if msg['acquired']: continue
            if msg['topic'] != topic: continue
            if msg['delivery_time'] > now: continue
